...
def query():
    ...
    os.makedirs('/tmp/tender_uploads', exist_ok=True)
    
    # Save the file temporarily to process
    file_path = os.path.join('/tmp/tender_uploads', secure_filename(file.filename))
    
    try:
        # Save the file
        file.save(file_path)
        
        # Process the tender file and get the response
        response = tender_analyzer.analyze_tenderQA(file_path, query)
